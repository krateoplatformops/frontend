...
import type { EventsApiResource, RestApiResponse } from '../../utils/types'
  ...
  if (!requireConfirmation || window.confirm('Are you sure?')) {
    ...
    if (onEventNavigateTo) {
      ...
      const loadingMessage = onEventNavigateTo.loadingMessage
        ? await context.resolveJq(onEventNavigateTo.loadingMessage, { json: payload, response: jsonResponse })
      ...
      const resolveErrorDescription = async () => {
        ...
        if (errorMessage) {
          description = errorMessage.startsWith('${')
            ? await context.resolveJq(errorMessage, { json: payload, response: jsonResponse })
            : errorMessage
        }
        return description
      }
      ...
      const resolveRedirectUrl = async (eventData: EventsApiResource) => {
        if (onEventNavigateTo.url.startsWith('${')) {
          return context.resolveJq(onEventNavigateTo.url, {
      ...
      if (mode === 'notification') {
        ...
        context.message.loading(loadingMessage, onEventNavigateTo.timeout)
