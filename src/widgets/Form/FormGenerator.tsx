import {Anchor,Col,Form,FormInstance,Input,InputNumber,Radio,Row,Select,Slider,Space,Switch,Typography} from "antd";
...
const FormGenerator = ({title, 
  ...
  useEffect(() => {
				...
				return (
					<div id={name} className={styles.formField}>
						<Form.Item
							...
							extra={!descriptionTooltip && node.description ? node.description : undefined}
						>
							<ListEditor onChange={(values) => {  form.setFieldValue(name.split("."), values) }} />
						</Form.Item>
					</div>
				)
			...
			if (currentProperties) {
				return Object.keys(currentProperties).map(key => {
					...
					if (currentProperties[key].type === "object") {
						...
						return {
							key: currentName,
							title: <span key={key} className={styles.anchorObjectLabel}>{label}</span>,
	...
	const updateJson = (values: any, keyPath: string, valuePath: string) => {
		const getObjectByPath = (obj: any, path: string) =>
